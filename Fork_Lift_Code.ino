/*
  ESP8266 WiFi Forklift Car (AP Web UI)
  - L298N drive: Car movement + Forklift up/down
...
const uint8_t LIFT_IN1 = D5;  // Up/Down direction
...
document.addEventListener('visibilitychange',()=>{if(document.hidden){endHold(document.querySelector('.pressed'));}});
...
window.addEventListener('load',()=>{
  ...
  bindHold('btnUp','/liftup','Lift Up');
  bindHold('btnDn','/liftdown','Lift Down');
});
</script>
</head>
<body>
...
void handleLiftUp(){ liftUp(); noteMotion(); server.send(200,"text/plain","OK"); }
void handleLiftDown(){ liftDown(); noteMotion(); server.send(200,"text/plain","OK"); }
