/*
  ...
  - Optimized for fast button response
*/
...
#define HX711_SCK  D8
HX711 scale;
float calibration_factor = -132520.0;  // Adjust for your load cell
...
function startHold(btn,cmd){if(btn)btn.classList.add('pressed');call(cmd);keepaliveTimer=setInterval(()=>{call(cmd);},100);}
...
<body>
  ...
  <div class="grid">
    ...
    <button class="tare wide" onclick="fetch('/tare')">Tare (Zero)</button>
  </div>
...
void handleTare(){scale.tare();Serial.println("TARE executed via web button.");server.send(200,"text/plain","TARE OK");}
void handleWeight(){float w = scale.get_units(1); String msg = String(w,2); server.send(200,"text/plain",msg);}
...
void loop(){
  ...
  // Print weight on Serial Monitor every second (fast reading)
